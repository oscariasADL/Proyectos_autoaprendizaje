...
(() =>{
    ...
    getRequest(`https://swapi.dev/api/people/74/`).pipe(
        ...
        /*switchMap( personaje => {
            ...
            return getRequest(especieUrl).pipe(
                map( especie => ({ personaje, especie }) )
            );
        })*/


        switchMap(
            personaje => zip( of(personaje),  getRequest( personaje.species[0] ) )
        ),
