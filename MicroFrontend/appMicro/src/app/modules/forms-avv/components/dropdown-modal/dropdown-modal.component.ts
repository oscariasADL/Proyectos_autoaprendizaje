...
import { Items } from './dropdown-modal.interface';

let $id = 0;

@Component({
  selector: 'avv-dropdown-modal',
  templateUrl: './dropdown-modal.component.html',
  styleUrls: ['./dropdown-modal.component.sass']
})
export class DropdownModalComponent implements AfterViewInit {
  ...
  public async touchEnd(): Promise<void> {
    if (!this.disabled && !this.loading) {
      if (this.isFocused) {
      ...
      const modal = await this.modalCtrl.create({
        ...
        componentProps: {
          ...
          isTypeRadioButton: this.isTypeRadioButton
        },
