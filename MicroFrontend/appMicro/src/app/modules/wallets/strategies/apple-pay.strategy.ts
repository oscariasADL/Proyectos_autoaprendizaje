import { DigitalWalletStrategy } from '@modules/wallets/entities/digital-wallet-strategy.interface';
import { Injectable } from '@angular/core';
import { DigitalWallet } from '@commons/capacitor-web-plugins/digital-wallet';

@Injectable()
export class ApplePayStrategy
  implements Omit<DigitalWalletStrategy, 'addListener'>
{
  public async validateWalletStatus(): Promise<void> {
    return DigitalWallet.validateWalletStatus();
  }

  public initializeWalletProvisioning(): Promise<void> {
    return DigitalWallet.initializeWalletProvisioning();
  }

  public checkEligibility(): Promise<void> {
    return DigitalWallet.checkEligibility();
  }

  public createWallet(options: {
    activationCode: string;
    phoneNumber?: string;
  }): Promise<void> {
    return DigitalWallet.createWallet({
      activationCode: options.activationCode
    });
  }

  public isWalletCreated(): Promise<{ wallet: boolean }> {
    return DigitalWallet.isWalletCreated();
  }

  public async getWalletId(): Promise<{ walletId: string }> {
    const isCreated = await DigitalWallet.isWalletCreated();
    if (!isCreated) {
      throw new Error('WALLETS.PROVISIONING.PROVISIONING_ERROR');
    }

    const walletId = await DigitalWallet.getWalletId();

    if (!walletId?.walletId) {
      throw new Error('WALLETS.PROVISIONING.PROVISIONING_ERROR');
    }

    return walletId;
  }

  public enrollCardToWallet(options: {
    enrollmentData: string;
  }): Promise<{ success: string }> {
    return DigitalWallet.enrollCardToWallet({
      enrollmentData: options.enrollmentData
    });
  }

  public getDigitalCardId(options: {
    cardId: string;
  }): Promise<{ digitalCardId: string }> {
    return DigitalWallet.getDigitalCardId({ cardId: options.cardId });
  }

  public setCustomCardDisplay(options: {
    cardId: string;
    cardImageUrl: string;
    cardDescription: string;
  }): Promise<{ imageIsLoaded: boolean }> {
    return DigitalWallet.setCustomCardDisplay({
      cardId: options.cardId,
      cardImageUrl: options.cardImageUrl,
      cardDescription: options.cardDescription
    });
  }

  public getDigitalCards(): Promise<{ cards: string }> {
    return DigitalWallet.getDigitalCards();
  }

  public getStatusPlatformService(options: {
    cardId: string;
  }): Promise<{ status: string }> {
    return DigitalWallet.getStatusApplePayService({ cardId: options.cardId });
  }

  public async canPushCardInWalletPay(options: {
    cardId: string;
  }): Promise<{ canPushCardWalletPay: boolean }> {
    const result = await DigitalWallet.canPushCardInApplePay({
      cardId: options.cardId
    });
    return { canPushCardWalletPay: result?.canPushCardInApplePay };
  }

  public async pushCardToWalletPay(options: {
    cardId: string;
  }): Promise<{ pushToWalletPay: boolean }> {
    const result = await DigitalWallet.pushCardToApplePay({
      cardId: options.cardId
    });
    return { pushToWalletPay: result?.pushToApplePay };
  }
}
