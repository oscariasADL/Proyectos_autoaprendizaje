...
import {
  POCKET_CREATE_AVAILABLE_FIELD,
  PocketCreateSlide
} from '@modules/pockets/pages/pocket-create/constants/pocket-create.constants';

export function pocketNameValidators(control: UntypedFormControl): {
  [key: string]: boolean;
} {
...
export function pocketGoalValidators(control: UntypedFormControl): {
  [key: string]: boolean;
} {
...
export function pocketQuotaValidators(control: UntypedFormControl): {
  [key: string]: boolean;
} {
  const value: number = control.currencyValue();
  const formGroup = this.form;
...
export function pocketOpenAmountValidators(control: UntypedFormControl): {
  [key: string]: boolean;
} {
  const value: number = control.currencyValue();
  const formGroup = this.form;
  if (!isNullOrUndefined(formGroup) && !isNullOrUndefined(value)) {
    ...
    if (value > this.facade.boundsByKey(ParameterKey.pocketOpenAmountMax)) {
      return { pocketCreateOpenAmountMax: true };
    }

    if (!isNullOrUndefined(goal) && value >= goal) {
      return { pocketCreateOpenAmountNotValid: true };
    }

    if (
      !isNullOrUndefined(product) &&
      value > product[POCKET_CREATE_AVAILABLE_FIELD]
    ) {
      return { pocketCreateOpenAmountNotAllowed: true };
    }
  }
  return null;
}

export function pocketProductValidators(control: UntypedFormControl): {
  [key: string]: boolean;
} {
  const formGroup = this.form;
  if (!isNullOrUndefined(formGroup)) {
    formGroup
      .get('openAmount')
      .setValidators([pocketOpenAmountValidators.bind(this)]);
    formGroup.get('name').updateValueAndValidity();
    formGroup.get('openAmount').updateValueAndValidity();
  }
  return null;
}
